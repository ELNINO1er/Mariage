...
export async function deleteMemoryAction(input:{id:string;kind:"photo"|"entry"}){const parsed=z.object({id:z.string().cuid(),kind:z.enum(["photo","entry"])}).safeParse(input);if(!parsed.success)return{ok:false,error:"Action invalide"};const membership=await editor();if(parsed.data.kind==="entry"){const result=await prisma.guestBookEntry.deleteMany({where:{id:parsed.data.id,weddingId:membership.weddingId}});revalidatePath("/dashboard/memories");return{ok:Boolean(result.count)};}
  const photo=await prisma.galleryPhoto.findFirst({where:{id:parsed.data.id,weddingId:membership.weddingId},select:{id:true,imageUrl:true}});if(!photo)return{ok:false,error:"Photo introuvable"};await prisma.galleryPhoto.delete({where:{id:photo.id}});const root=path.resolve(process.cwd(),"public","uploads","gallery");const target=path.resolve(process.cwd(),"public",photo.imageUrl.replace(/^\/+/,""));if(target.startsWith(`${root}${path.sep}`))await unlink(target).catch(()=>undefined);revalidatePath("/dashboard/memories");return{ok:true};}
